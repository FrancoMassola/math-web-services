...
const router = Router();
const myModule = require('../math_functions/math_functions');
//router GET 
router.get('/:expression/:presicion', (req,res)=>{
    ...
    if (string_expression.indexOf('s')!=-1) {
        ...
        var math_resultSr = myModule.square_root_function(string_expression);
        //function call to get significant digits
        var significant_digits = myModule.get_significantDigits(math_resultSr, string_presicion);
       ...
       res.send("The result of math expression is: " + math_resultSr + " --- The significant digits are: " + significant_digits);
          ...
          var math_result = myModule.math_expression_result(string_expression);
          ...
          //function call to get significant digits
          significantDigits = myModule.get_significantDigits(math_stringR,string_presicion);
          //send response to the client
...
router.post('/', (req,res)=>{
    ...
    if (stringExpression.indexOf('s')!=-1) {
      var mathResult = myModule.square_root_function(stringExpression);
      var significantDigits = myModule.get_significantDigits(mathResult, sting_significantDigits);
     res.send("The result of math expression is: "  + mathResult + " --- The significant digits are: " + significantDigits);
        ...
        var finalResult = myModule.math_expression_result(stringExpression);
        var stringMath_result = (new String(finalResult)).valueOf();
        var digits_notRoot_result = '';
        console.log("Resultado final: " + finalResult);
        //function call to get significant digits
        digits_notRoot_result = myModule.get_significantDigits(stringMath_result,sting_significantDigits);
